...
namespace ProDashBoard.Data
{
    public class CD_CustomerSatisfactionRepository : ICD_CustomerSatisfactionRepository
    {
        ...
        {
            List<CD_ProjectSatisfactionData> resultsArray = this._db.Query("SELECT  pj.Name, sumr.Rating FROM CustomerSatisfactionSummary sumr join Project pj on sumr.ProjectId = pj.Id where sumr.Year = " + year + " and sumr.Quarter =" + quarter + " and sumr.Rating !=0 order by ABS(sumr.Rating) desc; ").Select(d => new CD_ProjectSatisfactionData { Name = d.Name, Rating = d.Rating }).ToList();
