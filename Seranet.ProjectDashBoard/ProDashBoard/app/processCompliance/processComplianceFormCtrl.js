(function () {
    ...
    app.controller('processComplianceFormController', ['$scope', 'toaster', '$mdDialog', '$window', '$http', '$stateParams', processComplianceFormController]);

    //angularjs controller method
    function processComplianceFormController($scope, toaster, $mdDialog, $window, $http, $stateParams) {
        $scope.answerArray = [];
        isAuthorized();
        initializer();
        isAdminOrTeamLead();
    ...
    .error(function () {
        ...
        function initializer() {
            $http.get('api/AppSettings/getProcessComplianceVersion').success(function (data) {
                $scope.isoLevelText = data;
            })
        ...
        $scope.submitFormData = function () {
            ...
            $scope.qParameters = [];
            $scope.qParameters.push("");
            $scope.qParameters.push("Consistent");
            $scope.qParameters.push("Inconsistent");
            $scope.qParameters.push("Poor");
            $scope.initializeQualityParameterCombo();
            $http.get('api/ProcessCompliance/getQualityParameters').success(function (data) {
                $scope.originalParameters = data;
                $scope.QualityParameters = chunk(data);
            })
        ...
        function submitReceivedData() {
            ...
            $scope.sendingResults = project.Id + "|" + project.AccountId + "|" + $scope.yearCombo + "|" + $scope.quarterCombo + "|" + $scope.rating + "|" + $scope.isoLevelText + "|";
            for (var y = 0; y < $scope.originalParameters.length; y++) {
